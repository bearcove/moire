...
use peeps_types_legacy::GraphSnapshot;
    ...
    loop {
        ...
        match msg {
            DashboardFrame::Handshake => {
                ...
                let proc_key = peeps_types_legacy::make_proc_key(&hello.process, hello.pid);
            ...
            DashboardFrame::GraphReply => {
                ...
                let proc_key = peeps_types_legacy::make_proc_key(&reply.process, reply.pid);
                apply_connection_identity(
                    state,
                    conn_id,
                    proc_key.clone(),
            ...
            DashboardFrame::ClientError => {
                ...
                apply_connection_identity(
                    state,
                    conn_id,
                    peeps_types_legacy::make_proc_key(&msg.process, msg.pid),
                    msg.process.clone(),
                    Some(msg.pid),
                )
                .await;
