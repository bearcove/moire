...
pub(crate) fn emit_into_graph(graph: &mut GraphSnapshot) {
    ...
    for weak in lock_infos.iter() {
        ...
        graph.nodes.push(crate::registry::make_node(
            ...
            peeps_types_legacy::NodeKind::Lock,
...
fn lock_detail_level(name: &str) -> &'static str {
    if name.starts_with("ConnectionHandle.") {
        peeps_types_legacy::InstrumentationLevel::Debug.as_str()
    } else {
        peeps_types_legacy::InstrumentationLevel::Info.as_str()
    }
}
...
impl LockInfo {
    ...
    fn new(name: &'static str) -> Arc<Self> {
        ...
        let info = Arc::new(Self {
            name,
            endpoint_id: peeps_types_legacy::new_node_id("lock"),
