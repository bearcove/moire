...
use facet::Facet;
use peeps_types_legacy::{Edge, EdgeKind, NodeKind};
...
pub fn channel<T>(name: impl Into<String>, buffer: usize) -> (Sender<T>, Receiver<T>) {
    ...
    let tx_node_id = peeps_types_legacy::new_node_id("mpsc_tx");
    let rx_node_id = peeps_types_legacy::new_node_id("mpsc_rx");
...
pub fn oneshot_channel<T>(name: impl Into<String>) -> (OneshotSender<T>, OneshotReceiver<T>) {
    ...
    let caller = std::panic::Location::caller();
    let location = crate::caller_location(caller);
    let tx_node_id = peeps_types_legacy::new_node_id("oneshot_tx");
    let rx_node_id = peeps_types_legacy::new_node_id("oneshot_rx");
...
pub fn watch_channel<T: Send + Sync + 'static>(
    ...
    let caller = std::panic::Location::caller();
    let location = crate::caller_location(caller);
    let tx_node_id = peeps_types_legacy::new_node_id("watch_tx");
    let rx_node_id = peeps_types_legacy::new_node_id("watch_rx");
...
pub(super) fn emit_channel_nodes(graph: &mut peeps_types_legacy::GraphSnapshot) {
    let now = Instant::now();
        ...
        for info in reg.iter().filter_map(|w| w.upgrade()) {
            ...
            // TX node
            {
                let attrs = MpscTxAttrs {
                    ...
                    peeps_level: peeps_types_legacy::InstrumentationLevel::Debug.as_str(),
                };
