...
pub use peeps_types_legacy::{self as types, GraphSnapshot, IntoMetaValue, MetaBuilder, MetaValue};
pub use peeps_types_legacy::{canonical_id, meta_key};
...
pub use peeps_types_legacy::InstrumentationLevel;
...
pub fn init(process_name: impl Into<String>) {
    ...
    let proc_key = peeps_types_legacy::make_proc_key(&name, pid);
    registry::init(&name, &proc_key);
    peeps_types_legacy::set_process_name(&name);
    #[cfg(feature = "dashboard")]
    {
