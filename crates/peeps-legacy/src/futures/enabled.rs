...
use peeps_types_legacy::{GraphSnapshot, InstrumentationLevel, NodeKind};
...
impl<F> Drop for PeepableFuture<F> {
    fn drop(&mut self) {
        ...
        // Clean up any touch edges from and to this future.
        crate::registry::remove_touch_edges_from(&self.node_id);
        crate::registry::remove_touch_edges_to(&self.node_id);
    ...
    F: IntoFuture,
{
    __peeps_track_future_with_meta_kind_level(
        ...
        peeps_types_legacy::MetaBuilder::new(),
    )
}
...
pub fn __peeps_track_future_with_meta<F>(
    ...
    meta: peeps_types_legacy::MetaBuilder<'_>,
...
pub fn __peeps_track_future_with_meta_kind_level<F>(
    ...
    mut meta: peeps_types_legacy::MetaBuilder<'_>,
) -> PeepableFuture<F::IntoFuture>
where
    F: IntoFuture,
{
    meta.push(
        peeps_types_legacy::meta_key::PEEPS_LEVEL,
        peeps_types_legacy::MetaValue::Static(level.as_str()),
    );
    let node_id = peeps_types_legacy::new_node_id(kind.as_str());
...
{
    let name = name.into();
    let node_id = peeps_types_legacy::new_node_id("future");
    let caller = std::panic::Location::caller();
    let location = crate::caller_location(caller);
...
pub fn sleep(
    ...
    crate::peep!(
        ...
        level = peeps_types_legacy::InstrumentationLevel::Debug,
        kind = peeps_types_legacy::NodeKind::Future,
        {
